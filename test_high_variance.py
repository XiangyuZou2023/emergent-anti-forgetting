"""
ADHD完整模拟：被动保护(niche) + 主动探索(高方差突变)
Compares: standard vs niche-only vs high-variance-only vs niche+high-variance
"""
...
class Genome:
    ...
    def mutate(self, prob=0.4, scale=0.2):
        """prob: chance per gene to mutate. scale: how far it jumps."""
...
def run_exp(label, niche_grace, fit_pct, mut_prob, mut_scale, random_inject, seed):
    """random_inject = how many pure random individuals injected per gen"""
    ...
    for gen in range(GEN):
        ...
        while len(new_pop) < POP:
            ...
            c.mutate(mut_prob, mut_scale)
            new_pop.append(c)
        # Inject random fresh blood (ADHD: never fully converge)
        while len(new_pop) < POP:
            new_pop.append(Genome())
        # Extra random injection
...
# ========================================
# Configs: (label, niche_grace, fit_pct, mut_prob, mut_scale, random_inject)
variants = [
    ('standard',           0, 30, 0.40, 0.20, 0),  # original
    ('niche_div5+cut40',   5, 40, 0.40, 0.20, 0),  # niche only (best from before)
    ('high_var_only',      0, 30, 0.70, 0.40, 5),  # high variance only, no niche
    ('high_var+cut40',     0, 40, 0.70, 0.40, 5),  # high var + tougher cut
    ('full_adhd_div5+40%', 5, 40, 0.70, 0.40, 5),  # niche + high var + tough cut
]
SEEDS = [42, 123]

print(f'{"label":<22s} {"vocab":>5s} {"sr":>6s} {"A*":>6s} {"accA":>6s} {"retA":>6s} {"forget":>7s} {"alive%":>6s}')
print('-' * 75)
for label, grace, pct, mp, ms, rj in variants:
    results = []
    for seed in SEEDS:
        r = run_exp(label, grace, pct, mp, ms, rj, seed)
        results.append(r)
    avg = lambda k: np.mean([r[k] for r in results])
    print(f'{label:<22s} {avg("vocab"):>5.0f} {avg("sr"):>6.3f} {avg("A"):>6.3f} '
