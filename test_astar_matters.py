"""
Test: Does A* gate actually matter?
Uses DETERMINISTIC A* measurement — only genuine recurrent dynamics survive.
Compares FULL (A* gate) vs NO_ASTAR (no gate) evolution.
Key metric: does no_astar drift to low-sr architectures that FORGET?
"""
...
# ═══ Model with separate noisy/det step ═══
...
# ═══ Measure (A→B→A with deterministic A*) ═══
...
def run(cfg_name, use_astar, seed):
    ...
    for gen in range(GEN):
        ...
        if scores[idx] > best_fit: best_fit = scores[idx]
    ...
    return {
        ...
        'med_A': round(final_A, 3),
    }

# ═══ RUN ═══
print("=" * 75)
print("TEST: Deterministic A* — does removing A* gate cause population drift?")
print("=" * 75)
...
for cfg_name in ['WITH_A*', 'NO_A*']:
    rows = [r for r in results if r['cfg'] == cfg_name]
    avg = lambda k: np.mean([r[k] for r in rows])
    print(f"  {cfg_name}: sr={avg('med_sr'):.3f} A*={avg('med_A'):.3f} "
          f"forget={avg('forget'):+.4f} alive%={avg('alive_pct'):.0f}")

print("\nKEY INSIGHT:")
print("  WITH A*: only high-sr architectures survive → anti-forgetting")
print("  NO A*: population drifts to low-sr → architectures that FORGET")
