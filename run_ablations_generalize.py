...
# Split characters for generalization test
mid = len(all_chars) // 2
...
def make_taskAB(vocab_size, char_set=None):
    if char_set is None:
        char_set = all_chars
...
# ═══════════════════════════════════════════════════════════════
def train_only(emb, ro, core, opt, X, Y, n_train, B):
    """Train emb+ro on (X, Y) for n_train steps. Core weights unchanged."""
    perm = torch.randperm(len(X))[:n_train]
    for i in range(0, len(perm), B):
        idx = perm[i:i+B]
        bx, by = X[idx].to(DEV), Y[idx].to(DEV)
        e = emb(bx).flatten(1)
        hh = torch.zeros(len(bx), core.W.shape[0], device=DEV)
        for _ in range(3): hh = core.step(hh, e)
        loss = F.cross_entropy(ro(hh), by)
        opt.zero_grad(); loss.backward(); opt.step()

@torch.no_grad()
def test_only(emb, ro, core, X, Y, B=64):
    """Evaluate accuracy on (X, Y). No training."""
    correct, total = 0, 0
...
# ═══════════════════════════════════════════════════════════════
def measure(ind, X_train, YA_train, YB_train, X_test, YA_test, YB_test, vocab, cfg, B=32):
    """
    MODIFIED: train on train_chars, compute fitness on test_chars.
    cfg: dict with keys:
    ...
    opt_core = torch.optim.Adam(list(emb.parameters()) + list(ro.parameters()), lr=0.01)

    # TRAIN on train chars
    train_only(emb, ro, core, opt_core, X_train, YA_train, 200, B)
    train_only(emb, ro, core, opt_core, X_train, YB_train, 200, B)

    # FITNESS = generalization on TEST chars (unseen characters!)
    acc_A = test_only(emb, ro, core, X_test, YA_test, B)
    acc_B = test_only(emb, ro, core, X_test, YB_test, B)

    # Ret: retrain briefly on A, re-test on test chars
    opt_ret = torch.optim.Adam(list(emb.parameters()) + list(ro.parameters()), lr=0.01)
    train_only(emb, ro, core, opt_ret, X_train, YA_train, 50, B)
    acc_A_ret = test_only(emb, ro, core, X_test, YA_test, B)
...
def run_evolution(cfg, seed=42):
    ...
    POP, GEN = 20, 30  # reduced for quick test
    ...
    for gen in range(GEN):
        ...
        for ind in pop:
            A, fit, aA, aB, aR, h_val = measure(ind, X_tr, YA_tr, YB_tr, X_te, YA_te, YB_te, V, cfg)
            scores.append(fit)
            As.append(A)
            accAs.append(aA)
            accBs.append(aB)
